...
export default function PredatorDashboard() {
  ...
  const isProcessingAnyTranscriptRef = useRef(false); // Global lock to prevent concurrent transcript processing
  const prevKeyHighlightsRef = useRef(null); // Track previous keyHighlights to prevent unnecessary re-renders
  const prevSentimentDataRef = useRef(null); // Track previous sentimentData to prevent unnecessary re-renders
  ...
  const processCustomerInfo = async (transcript) => {
    ...
    try {
      ...
      if (!extractedData || (!extractedData.email && !extractedData.name && !extractedData.phone && !extractedData.company)) {
        // If no customer info found but we already have customerData, keep it
        // Key highlights and sentiment will still update silently
        return;
      }
      ...
      if (extractedData && (extractedData.email || extractedData.name || extractedData.phone || extractedData.company)) {
        ...
        if (extractedData.email) {
          newCustomerEmail = extractedData.email;
          ...
          try {
            ...
            if (existingData) {
              // Check if this is a different customer - if so, replace customerData
              // If same customer, keep existing data and just update silently
              const isDifferentCustomer = customerData && customerData.email && customerData.email !== existingData.email;
              
              if (isDifferentCustomer || !customerData) {
                // New customer or first time - update customerData
                ...
                // Same customer - keep existing data, just update HubSpot silently
                customerFound = true;
              }
              ...
              if (extractedData.name || extractedData.phone || extractedData.company) {
                try {
                  ...
                  // Only show toast if it's a new customer
                  if (!customerData || (customerData.email !== existingData.email)) {
              ...
              // Customer not found in HubSpot - check if we have existing customerData
              // Only clear if we were explicitly searching for this exact customer
              if (customerData && customerData.email && customerData.email === extractedData.email) {
                // Same email but not found - might have been deleted, clear it
                setCustomerData(null);
                setCrmSidebarVisible(false);
              }
              // Otherwise keep existing customerData and sidebar visible
              // This ensures sidebar stays open for existing customer while key highlights/sentiment update
            }
          } catch (error) {
            // Don't clear customerData on HubSpot search errors - keep existing data
          }
          ...
          try {
            ...
            if (searchResult && searchResult.customers && searchResult.customers.length > 0) {
              ...
              const isDifferentCustomer = customerData && customerData.email && customerData.email !== existingCustomer.email;
              ...
              if (isDifferentCustomer || !customerData) {
                ...
                // Same customer - keep existing data
                customerFound = true;
              }
              ...
              if (extractedData.name || extractedData.phone || extractedData.company) {
                try {
                  await updateCustomerData(extractedData);
                  // Only show toast if it's a new customer
                  if (!customerData || (customerData.email !== existingCustomer.email)) {
                    showToast("Customer data updated in HubSpot CRM");
                  }
                } catch (updateError) {
                }
              ...
              // Customer not found - keep existing customerData if it exists
              // Only clear if we were explicitly looking for this customer and they don't exist
            }
          } catch (error) {
            // Don't clear customerData on HubSpot search errors - keep existing data
          }
      ...
      // Don't clear customerData if we already have it and no new customer was explicitly found
      // This allows key highlights and sentiment to update silently while preserving customer data
      
    } catch (error) {
      // Don't clear customerData on errors - preserve existing data
      // Only show error if we don't have customerData yet
      if (!customerData) {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === 'ready') {
            ...
            if (t) {
              // Only update latestWsTranscriptRef with FINAL transcripts to avoid partial/duplicate text
              if (data.isFinal) {
                latestWsTranscriptRef.current = t; // capture latest from WS immediately
                console.log('🎤 FRONTEND: Final transcript received:', t);
              }
              // Always update UI for live display (both interim and final)
    ...
    const recordAndProcess = async () => {
      try {
        ...
        mediaRecorder.onstop = async () => {
          ...
          // Global lock check - prevent concurrent transcript processing
          if (isProcessingAnyTranscriptRef.current) {
            console.log('🎤 FRONTEND: Another transcript is being processed, skipping');
            return;
          }
          
          // Set all locks BEFORE the try block to ensure they're always in finally
          isProcessingInOnstop = true; // Mark as processing
          isProcessingAnyTranscriptRef.current = true; // Set global lock
          ...
          try {
            ...
            // Deduplication: Skip if this transcript was already processed
            if (lastProcessedTranscriptRef.current === latestWsTranscriptRef.current.trim()) {
              console.log('🎤 FRONTEND: Duplicate transcript detected, skipping processing:', latestWsTranscriptRef.current);
              processedSpeech = true;
              return;
            }
            ...
            // Start timer BEFORE API call to measure actual response time
            const pipelineTimer = startSpeedTimer();
            ...
            try {
              ...
              // Log actual API response time
              const apiTime = endSpeedTimer(pipelineTimer);
              ...
              console.log(`⏱️ API Response Time (Error): ${apiTime}ms`);
            ...
            if (transcript && transcript.trim()) {
              ...
              if (JSON.stringify(prevSentimentDataRef.current) !== JSON.stringify(sentimentData)) {
                setSentimentData(sentimentData);
                prevSentimentDataRef.current = sentimentData;
              }
              // Update key highlights from pipeline (only set if changed to prevent unnecessary re-renders)
              const currentKeyHighlights = keyHighlights !== undefined && keyHighlights !== null ? keyHighlights : {};
              if (JSON.stringify(prevKeyHighlightsRef.current) !== JSON.stringify(currentKeyHighlights)) {
                setKeyHighlights(currentKeyHighlights);
                prevKeyHighlightsRef.current = currentKeyHighlights;
              }
              ...
              if (responseText && responseText.trim()) {
                try {
                  ...
                  if (suggestions.length > 0) {
                    ...
                    // Mark this transcript as processed to prevent reprocessing
            ...
            isProcessingAnyTranscriptRef.current = false; // Release global lock
          }
        };
        ...
        const checkSpeechAndSilence = () => {
          ...
          if (average > 28 && !speechDetected && !isRecording) {
            ...
            lastProcessedTranscriptRef.current = ""; // Reset processed transcript to allow new recording
